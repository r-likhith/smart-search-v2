...
const { deduplicateCategories } = require('./deduplicateCategories'); // ✅
...
async function getSuggestions(query, options = {}) {
  try {
    ...
    // deduplicate: catalogue (L1) preferred over category (L2) ✅
    const deduplicatedCategories = deduplicateCategories(categorySuggestions);
